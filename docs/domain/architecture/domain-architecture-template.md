...
export class TemplatePath {
  ...
  static create(path: string): Result<TemplatePath, TemplatePathError> {
    ...
    // サポートされる拡張子
    const validExtensions = ['.json', '.yaml', '.yml', '.toml', '.hbs', '.mustache'];
...
export class TemplateVariable {
  ...
  getDefaultValue(): unknown { return this.defaultValue; }
}
...
export class RenderedContent {
  ...
  static create(
    ...
    if (!content) {
      return {
        ok: false,
        error: { kind: "EmptyInput", message: "Rendered content cannot be empty" }
      };
    }
  ...
  getMetadata(): RenderMetadata { return { ...this.metadata }; }
}
...
export class Template {
  ...
  getState(): TemplateState { return this.state; }
    ...
    if (this.state.kind !== "Compiled") {
      return {
        ...
        error: {
          ...
          message: `Template is not compiled, current state: ${this.state.kind}`
        }
      };
    }
...
export interface CompiledTemplate {
  ...
  readonly render: (data: Record<string, unknown>) => Result<string, RenderError>;
  readonly variables: TemplateVariable[];
  readonly outputFormat: OutputFormat;
}
```
...
export class TemplateParser {
  ...
  private extractVariables(content: string): Result<TemplateVariable[], ParseError> {
  ...
  private detectOutputFormat(content: string, format: TemplateFormat): OutputFormat {
    // Template形式から出力形式を推定
    switch (format) {
...
export interface TemplateRepository {
  ...
  findByFormat(format: TemplateFormat): Promise<Result<Template[], TemplateError>>;
}
```
...
export type CompileError =
  | { kind: "CompilationFailed"; reason: string; message: string };

export type RenderError =
  | { kind: "RenderFailed"; reason: string; variable?: string; message: string }
  | { kind: "RequiredVariableMissing"; variable: string; path: string; message: string }
  | ResolutionError;

export type ResolutionError =
  | { kind: "PathNotFound"; path: string; segment: string; message: string }
  | { kind: "PropertyNotFound"; path: string; property: string; message: string };
