...
Schemaはフロントマターの解析構造を決め、テンプレートはフロントマターから得られた値の出力形式を決める。Schema解析結果がテンプレートの変数へ埋め込まれることで実現する。
...
b.抽出した情報を、「解析用Schema」で指定されたテンプレートを使って、出力テンプレートへ当て込む
