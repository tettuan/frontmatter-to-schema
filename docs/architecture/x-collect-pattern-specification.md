...
> **Usage Guide**: 使い方とサンプルコードは
> [x-collect-pattern Usage Guide](../usage/x-collect-pattern.md)
> を参照してください。
...
**決定理由**:
「パターンにマッチするプロパティを収集する」という動作を直接的に表現する名称。既存の`x-derived-from`（他から派生）と類似の命名規則で一貫性がある。
...
- `{user_variables[0].key}`, `{user_variables[0].value}`
  で個別要素にアクセス可能
...
- `src/domain/schema/services/schema-directive-processor.ts` -
  yaml-schema-mapperで処理済み
...
詳細な使用例とサンプルコードは **[Usage Guide](../usage/x-collect-pattern.md)**
を参照してください。
