...
**File**: `src/domain/services/interfaces.ts` **Issue**: Returns
`FrontMatter | null` (partial function) **Solution**: Use discriminated union:
...
Multiple services use optional parameters (`?:`) which should be converted to
explicit presence/absence modeling.

## 3. Schema Types

Schema-related types should use exhaustive discriminated unions for different
schema types instead of generic `unknown`.
