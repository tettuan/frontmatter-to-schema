...
export type DomainError =
  ...
  | {
    readonly kind: "ProcessingFailed";
    ...
    if (!formatResult.ok) {
      return {
        ...
        error: {
          kind: "ValidationFailed",
          field: "format",
          reason: formatResult.error.message,
        },
      };
    }
...
export class TemplateFormat {
  ...
  isYAML(): boolean {
    return this.value === "yaml";
  }
  isXML(): boolean {
    return this.value === "xml";
  }
  isTOML(): boolean {
    return this.value === "toml";
  }
}
```
...
export class PipelineResult {
  ...
  static create(
    ...
    return {
      ok: true,
      data: new PipelineResult(processed, successful, failed, output, time, [
        ...errors,
      ]),
    };
  }
  ...
  getErrors(): ReadonlyArray<ProcessingError> {
    return this.errors;
  }
