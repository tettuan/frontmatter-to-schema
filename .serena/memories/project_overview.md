...
## Core Purpose
Deno-based DDD system using Claude AI (`claude -p`) for frontmatter extraction and schema-driven analysis.

**Stack**: Deno + TypeScript + JSR packages + Claude CLI
**Principles**: DDD, TDD, Totality, AI-complexity-control
...
### Processing Flow
1. File Discovery → 2. FrontMatter Extraction → 3. AI Analysis (2-stage) → 4. Template Mapping → 5. Registry Output
...
### AI Integration
- **Stage 1**: Information extraction via `claude -p`  
- **Stage 2**: Template application via `claude -p`
