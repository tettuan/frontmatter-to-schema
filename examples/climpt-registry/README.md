...
This example demonstrates the use of x-* extension properties in the frontmatter-to-schema tool.
...
- **Examples**:
  - `"x-derived-from": "commands[].c1"` - Extracts all c1 values from commands array
  - `"x-derived-from": "commands[].c2"` - Extracts all c2 values from commands array
  - `"x-derived-from": "commands[].c3"` - Extracts all c3 values from commands array
...
2. **Data Aggregation**:
   - `availableConfigs`: Automatically populated with unique c1 values from all commands
...
1. **No Manual Aggregation**: The `availableConfigs` array is automatically populated from commands
2. **DRY Principle**: No need to manually maintain lists of categories, actions, or targets
