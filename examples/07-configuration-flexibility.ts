...
Generate a valid documentation entry.
    ...
    const analysisProcessor: SchemaAnalysisProcessor<
      FrontMatterContent,
    ...
    console.log(
      `   Success rate: ${
        (result.summary.successfulFiles / result.summary.processedFiles * 100)
          .toFixed(1)
      }%`,
    );
    ...
    console.log(
      "ℹ️  Custom schema example completed with expected limitations",
    );
...
async function runtimeConfigExample() {
  ...
  for (const config of configurations) {
    try {
      ...
      const prompts = {
        extractionPrompt:
          `Extract information using schema: {{schema}}\nFrom: {{data}}`,
      ...
      console.log(
        `      ✅ ${config.name}: ${result.summary.processedFiles} files`,
      );
    } catch (error) {
  ...
  console.log(
    "   🎯 Demonstrates: Runtime reconfiguration without code changes",
  );
  console.log("");
}
 ...
 */
function abstractionValidationExample() {
