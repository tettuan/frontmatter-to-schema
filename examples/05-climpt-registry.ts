...
class MockClaudeClient {
  analyze(prompt: string): Promise<Record<string, unknown>> {
      ...
      tools: {
        ...
        commands: [
          ...
          },
          {
            ...
            usage:
              "climpt-spec analyze quality-metrics -f spec.md -o report.json",
            options: {
              input: ["MD"],
              adaptation: ["default"],
              input_file: [true],
          ...
          },
          {
            ...
            usage:
              "climpt-code create implementation -f design.md -a detailed -o src/",
            options: {
              input: ["MD"],
              adaptation: ["default", "detailed"],
              input_file: [true],
...
async function main() {
  ...
  if (registry.tools.commands.length > 0) {
    ...
    console.log(`\n📌 Sample command:`);
    console.log(
      `  ${sample.c1}-${sample.c2}-${sample.c3}: ${sample.description}`,
    );
  }
}
