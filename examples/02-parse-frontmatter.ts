...
if (frontMatter) {
  ...
  if (config) {
    console.log("\n⚙️  Configuration:");
    const inputFormats = Array.isArray(config.input_formats)
      ? config.input_formats.join(", ")
      : "none";
    const processingModes = Array.isArray(config.processing_modes)
      ? config.processing_modes.join(", ")
      : "none";
    console.log(`  Input Formats: ${inputFormats}`);
    console.log(`  Processing Modes: ${processingModes}`);

    if (
      config.supports && typeof config.supports === "object" &&
      ...
      console.log(
        `  Output Destination: ${supports.output_destination ? "✓" : "✗"}`,
      );
    }
  }

  const variables = frontMatter.get("variables") as
  ...
  const command = {
    ...
    options: {
      ...
      adaptation: Array.isArray(config?.processing_modes)
        ? config.processing_modes
        : ["default"],
      input_file: (config && typeof config.supports === "object" &&
          config.supports !== null &&
          (config.supports as Record<string, unknown>).file_input)
        ? [true]
        : [false],
      stdin: (config && typeof config.supports === "object" &&
          config.supports !== null &&
          (config.supports as Record<string, unknown>).stdin_input)
        ? [true]
        : [false],
      destination: (config && typeof config.supports === "object" &&
          config.supports !== null &&
          (config.supports as Record<string, unknown>).output_destination)
        ? [true]
        : [false],
    },
  };
