...
Deno.test("RegistryBuilder - should build registry from analysis results", () => {
  ...
  builder.addAnalysisResult(result1);
  builder.addAnalysisResult(result2);
...
Deno.test("RegistryBuilder - should sort commands and configs", () => {
  const builder = new RegistryBuilder();
  ...
  commands.forEach((cmd) => {
    builder.addAnalysisResult(new AnalysisResult("test.md", [cmd]));
  });
