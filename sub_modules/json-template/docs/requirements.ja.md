...
テンプレート内の `{variable.path}`
形式の変数を、JSONデータの対応する値に置換する。
...
**要求**: 変数が存在しない場合でも処理を継続できる柔軟性を提供する。

#### 動作仕様

1. **エラーログ出力**: 変数が存在しない場合、標準エラー出力にエラーメッセージを出力
2. **処理継続**: エラーがあっても処理を中断せず、結果を返却
3. **デフォルト値の設定**: 存在しない変数は空文字列 `""` または `null` に置換

#### 置換ルール

- **文字列コンテキスト**: 変数が文字列として使われている場合 → `""`（空文字列）
  ```json
  // テンプレート
  { "value": "{missing.variable}" }

  // 結果（missing.variableが存在しない場合）
  { "value": "" }
  ```

- **その他のコンテキスト**: 変数が値として使われている場合 → `null`
  ```json
  // テンプレート
  { "count": {missing.count} }  // 注: 引用符なし

  // 結果（missing.countが存在しない場合）
  { "count": null }
  ```
...
```
[json-template WARNING] Variable not found: options.input_file
  Template: /path/to/template.json
  Context: Processing variable at position 145
```

#### 利点

1. **部分的なデータでの動作**: すべてのプロパティが揃っていなくても処理可能
2. **柔軟な開発**: テンプレートとデータの同期が緩やかでも動作
3. **デバッグの容易性**: エラーログで問題を特定しつつ、結果を確認可能
4. **後方互換性**: 既存の完全なデータでは従来通り動作
