...
1. **エラーログ出力**:
   変数が存在しない場合、以下の条件で標準エラー出力にエラーメッセージを出力
   - 環境変数 `VERBOSE=true` または `VERBOSE=1` が設定されている
   - 環境変数 `LOG_LEVEL=debug` が設定されている
2. **処理継続**: エラーがあっても処理を中断せず、結果を返却
3. **デフォルト値の設定**: 存在しない変数は空文字列 `""` に置換
...
#### エラー出力形式（VERBOSE=true または LOG_LEVEL=debug の場合）
...
3. **デバッグの容易性**: VERBOSE/DEBUG時にエラーログで問題を特定
