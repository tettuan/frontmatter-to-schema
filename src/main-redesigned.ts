...
async function main() {
  ...
  if (args.length < 1) {
    console.error(
      "Usage: deno run --allow-read --allow-write --allow-run main-redesigned.ts [prompts-dir] [output-path]",
    );
    console.error(
      "Example: deno run --allow-read --allow-write --allow-run main-redesigned.ts .agent/climpt/prompts .agent/climpt/registry.json",
    );
    Deno.exit(1);
  }
  ...
  try {
    ...
    console.log(
      `⚙️  Available configs: ${registry.tools.availableConfigs.join(", ")}`,
    );
  } catch (error) {
    console.error(
    ...
    if (error instanceof Error) {
      if (
        ...
        console.error(
          "  - Verify that prompt files (*.md) are present in the directory",
        );
...
async function advancedExample() {
  ...
  try {
    ...
    console.log(
      "ℹ️  Advanced example failed (expected if custom files don't exist)",
    );
