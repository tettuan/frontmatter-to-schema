 ...
 */
export class GenericSchemaAnalyzer<TSchema, TResult> implements SchemaBasedAnalyzer<TSchema, TResult> {
  constructor(
  ...
  private prepareAnalysisPrompt(data: unknown, schema: TSchema, context: AnalysisContext): string {
  ...
  private transformResult(result: unknown, schema: TSchema): TResult {
    ...
    throw new Error(`Analysis result does not conform to schema: ${JSON.stringify(result)}`);
  }

  private isValidResult(result: unknown, schema: TSchema): boolean {
 ...
 */
export class SchemaGuidedTemplateMapper<TSource, TTarget> implements TemplateMapper<TSource, TTarget> {
  constructor(
    private readonly externalService: ExternalAnalysisService,
    private readonly prompts: PromptConfiguration
  ) {}

  async map(source: TSource, template: TTarget, schema?: unknown): Promise<TTarget> {
  ...
  private prepareMappingPrompt(source: TSource, template: TTarget, schema?: unknown): string {
    const sourceString = JSON.stringify(source, null, 2);
    const templateString = JSON.stringify(template, null, 2);
    const schemaString = schema ? JSON.stringify(schema, null, 2) : 'No schema provided';
  ...
  private validateAndTransform(result: unknown, originalTemplate: TTarget): TTarget {
...
export class SchemaAnalysisProcessor<TInput, TSchema, TOutput> {
  constructor(
    private readonly analyzer: SchemaBasedAnalyzer<TSchema, any>,
    private readonly mapper: TemplateMapper<any, TOutput>,
    private readonly schema: TSchema,
    private readonly template: TOutput
  ) {}

  async process(input: TInput, context: AnalysisContext = {}): Promise<ProcessingResult<TOutput>> {
    try {
      // Step 1: Schema-driven analysis
      const analysisResult = await this.analyzer.analyze(input, this.schema, context);
      
      // Step 2: Template mapping
      const mappedResult = await this.mapper.map(analysisResult, this.template, this.schema);
...
export class SchemaAnalysisFactory {
  static createAnalyzer<TSchema, TResult>(
    ...
    return new GenericSchemaAnalyzer<TSchema, TResult>(externalService, prompts);
  }
  ...
  static createMapper<TSource, TTarget>(
    ...
    return new SchemaGuidedTemplateMapper<TSource, TTarget>(externalService, prompts);
  }
  ...
  static createProcessor<TInput, TSchema, TOutput>(
    ...
    const analyzer = this.createAnalyzer<TSchema, any>(externalService, prompts);
    const mapper = this.createMapper<any, TOutput>(externalService, prompts);
