...
import type { DerivationRule } from "../../src/domain/aggregation/value-objects.ts";
...
export class ResultAssertions {
  ...
  static assertResultOk<T, E>(result: Result<T, E>, message?: string): T {
    ...
    if (!result.ok) {
      throw new Error(
        `Result assertion failed: ${message || "Expected success"}`,
      );
    }
  ...
  static assertResultError<T, E>(
    ...
    if (result.ok) {
      throw new Error(
        `Result assertion failed: ${message || "Expected error"}`,
      );
    }
    ...
    if (
      ...
      assertEquals(
        (result.error as { kind: string }).kind,
        expectedErrorKind,
  ...
  static assertResultStructure<T, E>(result: Result<T, E>) {
    ...
    if (result.ok) {
      ...
      assertEquals(
        ...
        undefined,
        "Result error should be undefined when ok=true",
      );
    } else {
      ...
      assertEquals(
        ...
        undefined,
        "Result data should be undefined when ok=false",
      );
    }
  }
}
...
export class AggregationAssertions {
  ...
  static assertNestedStructure(
    ...
    for (const path of expectedPaths) {
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        assertExists(
          current[part],
          `Expected path ${parts.slice(0, i + 1).join(".")} to exist`,
        );
        ...
        if (i < parts.length - 1) {
          assertEquals(
            typeof current[part],
            "object",
            `Expected ${parts.slice(0, i + 1).join(".")} to be object`,
          );
   ...
   */
  static assertDerivedFieldValues(
    derivedFields: Record<string, unknown> | undefined,
    fieldPath: string,
    expectedValues: unknown[],
  ) {
    assertExists(derivedFields, "Derived fields should exist");
    assertExists(
      derivedFields[fieldPath],
      `Expected derived field ${fieldPath} to exist`,
    );
    ...
    assertEquals(
      ...
      true,
      `Expected derived field ${fieldPath} to be array`,
    );

    const arrayValue = fieldValue as unknown[];
    for (const expectedValue of expectedValues) {
      assertEquals(
        arrayValue.includes(expectedValue),
        true,
        `Expected derived field ${fieldPath} to contain ${expectedValue}`,
      );
    }
  }
  ...
  static assertUniqueValues(values: unknown[], message?: string) {
    ...
    assertEquals(
      uniqueCheck.size,
      values.length,
      message || "Values should be unique",
    );
  }
   ...
   */
  static assertFrontmatterPartDetection(
    ...
    assertExists(result, "Result should exist");
    assertEquals(
      typeof result.itemCount,
      "number",
      "itemCount should be number",
    );
    assertEquals(
      result.itemCount,
      expectedItemCount,
      `Expected item count to be ${expectedItemCount}`,
    );

    if (!hasItems) {
      assertEquals(
        "items" in (result.aggregated as Record<string, unknown>),
        false,
        "Should NOT have 'items' property for ArrayBased processing",
      );
    }
  }
}
...
export class SchemaAssertions {
   ...
   */
  static assertSchemaTemplateInfo(
    ...
    if (expectedRulesCount > 0) {
      ...
      assertEquals(
        Array.isArray(templateInfo.rules),
        true,
        "Template info rules should be array",
      );
      assertEquals(
        (templateInfo.rules as unknown[]).length,
        expectedRulesCount,
        `Expected ${expectedRulesCount} rules`,
      );
    }
  }
  ...
  static assertDerivationRule(
    ...
    assertEquals(
      rule.getTargetField(),
      expectedTarget,
      `Expected target field ${expectedTarget}`,
    );
    assertEquals(
      rule.getSourceExpression(),
      expectedSource,
      `Expected source expression ${expectedSource}`,
    );
    assertEquals(
      rule.isUnique(),
      expectedUnique,
      `Expected unique flag ${expectedUnique}`,
    );
  }
}
...
export class PerformanceAssertions {
   ...
   */
  static async timeAsyncExecution<T>(
    fn: () => Promise<T>,
...
export class TemplateAssertions {
   ...
   */
  static assertTemplateApplication(
    result: Record<string, unknown>,
   ...
   */
  static assertTemplateVariables(
    result: Record<string, unknown>,
    ...
    for (const [key, expectedValue] of Object.entries(expectedVariables)) {
      ...
      if (typeof expectedValue === "string" && expectedValue.startsWith("{{")) {
        ...
        assertEquals(
          actualValue,
          expectedValue,
          `Expected template variable ${key} to equal ${expectedValue}`,
        );
      }
    }
  }
