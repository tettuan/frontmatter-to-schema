...
Deno.test("ResultAssertions", async (t) => {
  ...
  await t.step("should assert error correctly", () => {
    ...
    const error = ResultAssertions.assertError(errorResult, "ParseError");
    assertEquals(error.kind, "ParseError");
    if (error.kind === "ParseError") {
      assertEquals(error.input, "test input");
      assertEquals(error.details, "test message");
    }
  });
