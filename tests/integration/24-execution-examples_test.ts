...
import { FeatureDetector, ImplementationTracker, type FeatureCapabilities } from "../utils/feature-detection.ts";
...
Deno.test("24 Execution Examples Validation", async (t) => {
  ...
  if (orchestratorResult.isError()) {
    throw new Error(`Failed to create orchestrator: ${orchestratorResult.unwrapError().message}`);
  }
...
# Content`);
    ...
    await t.step("Example 2: Directory processing multiple files", async () => {
      ...
      await fileSystem.writeTextFile(`${testDir}/doc1.md`, `---
title: "Doc 1"
---
Content 1`);

      await fileSystem.writeTextFile(`${testDir}/doc2.md`, `---
title: "Doc 2"
---
Content 2`);
      ...
      await fileSystem.writeTextFile(`${testDir}/item-template.json`, JSON.stringify({
        "title": "{title}"
      }));
    ...
    await t.step("Example 3: Schema validation with required fields", async () => {
...
Content`);
    ...
    await t.step("Example 4: Frontmatter extraction with nested objects", async () => {
...
Content`);
    ...
    await t.step("Example 8: Empty frontmatter handling", async () => {
      ...
      await fileSystem.writeTextFile(`${testDir}/test.md`, `# Just Content
No frontmatter here`);
  ...
  await t.step("Template Processing (Examples 9-16)", async (t) => {
    ...
    await t.step("Example 9: {@items} expansion with template-items", async () => {
...
Content 2`);
    ...
    await t.step("Example 10: Variable substitution with dot notation", async () => {
...
Content`);
      ...
      await fileSystem.writeTextFile(`${testDir}/template.json`, JSON.stringify({
        "connection_string": "host={config.database.host}:{config.database.port}",
    ...
    await t.step("Example 11: Template format variations (JSON vs YAML)", async () => {
...
Content`);
      ...
      await fileSystem.writeTextFile(`${testDir}/template.yaml`, `title: "{title}"
items:
  - "{data}"`);
...
Content`);
    ...
    await t.step("Example 16: Multiple template formats in single pipeline", async () => {
...
Content`);
      ...
      // Write invalid JSON
      await fileSystem.writeTextFile(`${testDir}/schema.json`, `{
        "type": "object"
...
Content`);
    ...
    await t.step("Example 23: Invalid output path (permission denied)", async () => {
...
Content`);
    ...
    await t.step("Example 24: Complex error accumulation scenario", async () => {
      const testDir = "tmp/test-24-examples/example-24";

      // Create scenario with multiple potential issues
      await fileSystem.writeTextFile(`${testDir}/test1.md`, `---
title: "Valid Doc"
---
Content`);

      await fileSystem.writeTextFile(`${testDir}/test2.md`, `---
title: "Another Valid"
---
Content`);
