...
Deno.test({
  name: "CLI Integration: Basic Command Execution",
  ignore: true, // Temporarily disabled - CLI integration needs separate fixes
}, async (t) => {
  ...
  try {
    ...
    await t.step(
      "should process files with command line arguments",
      async () => {
        const result = await runCLI([
          "-i",
          join(TEST_DIR, "markdown"),
          "-s",
          join(TEST_DIR, "schema.json"),
          "-t",
          join(TEST_DIR, "template.json"),
          "-o",
          join(TEST_DIR, "output", "cli-output.json"),
        ]);
    ...
    await t.step("should support verbose mode", async () => {
      const result = await runCLI([
        "-i",
        join(TEST_DIR, "markdown", "test1.md"),
        "-s",
        join(TEST_DIR, "schema.json"),
        "-t",
        join(TEST_DIR, "template.json"),
        "-o",
        join(TEST_DIR, "output", "verbose-output.json"),
        "--verbose",
      ]);
    ...
    await t.step("should support -v flag for verbose", async () => {
      const result = await runCLI([
        ...
        join(TEST_DIR, "template.json"),
        "-o",
        join(TEST_DIR, "output", "verbose-v-output.json"),
        "-v",
      ]);
...
Deno.test({
  name: "CLI Integration: Configuration File Support",
  ignore: true, // Temporarily disabled - CLI integration needs separate fixes
}, async (t) => {
  ...
  try {
    ...
    await t.step("should process with configuration file", async () => {
      ...
      const config = {
        ...
        schema: {
          definition: JSON.parse(
            await Deno.readTextFile(join(TEST_DIR, "schema.json")),
          ),
          format: "json",
        },
        template: {
      ...
      // Verify output file was created
      const outputExists = await exists(
        join(TEST_DIR, "output", "config-output.json"),
      );
      assertEquals(outputExists, true);
    });

    await t.step(
      "should override config file with command line args",
      async () => {
        const result = await runCLI([
          "-c",
          join(TEST_DIR, "config.json"),
          "-o",
          join(TEST_DIR, "output", "override-output.json"),
        ]);
...
Deno.test({
  name: "CLI Integration: Error Handling",
  ignore: true, // Temporarily disabled - CLI integration needs separate fixes
}, async (t) => {
  ...
  try {
    ...
    await t.step("should handle non-existent input file", async () => {
      const result = await runCLI([
        ...
        join(TEST_DIR, "template.json"),
        "-o",
        join(TEST_DIR, "output", "error.json"),
      ]);
    ...
    await t.step("should handle non-existent schema file", async () => {
      const result = await runCLI([
        ...
        join(TEST_DIR, "markdown"),
        "-s",
        join(TEST_DIR, "nonexistent-schema.json"),
        "-t",
        join(TEST_DIR, "template.json"),
        "-o",
        join(TEST_DIR, "output", "error.json"),
...
Just content here.`,
      ...
      const result = await runCLI([
        ...
        join(TEST_DIR, "template.json"),
        "-o",
        join(TEST_DIR, "output", "no-fm-output.json"),
      ]);
...
Deno.test({
  name: "CLI Integration: Output Format Support",
  ignore: true, // Temporarily disabled - CLI integration needs separate fixes
}, async (t) => {
  ...
  try {
    ...
    await t.step("should generate JSON output", async () => {
      const result = await runCLI([
        ...
        join(TEST_DIR, "template.json"),
        "-o",
        join(TEST_DIR, "output", "test.json"),
      ]);
    ...
    await t.step("should generate YAML output", async () => {
      const result = await runCLI([
        ...
        join(TEST_DIR, "template.json"),
        "-o",
        join(TEST_DIR, "output", "test.yaml"),
      ]);
...
Deno.test({
  name: "CLI Integration: Directory Processing",
  ignore: true, // Temporarily disabled - CLI integration needs separate fixes
}, async (t) => {
  ...
  try {
    ...
    await t.step("should process entire directory", async () => {
      const result = await runCLI([
        ...
        join(TEST_DIR, "template.json"),
        "-o",
        join(TEST_DIR, "output", "directory-output.json"),
      ]);
    ...
    await t.step("should handle empty directory", async () => {
      ...
      const result = await runCLI([
        ...
        join(TEST_DIR, "template.json"),
        "-o",
        join(TEST_DIR, "output", "empty-output.json"),
      ]);
