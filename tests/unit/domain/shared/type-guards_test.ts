...
Deno.test("asObjectRecord - converts unknown to Record with validation", () => {
  ...
  if (!result3.ok) {
    ...
    const errorWithMessage = result3.error as any;
    assertEquals(errorWithMessage.message?.includes("Expected object but got"), true);
  }
  ...
  if (!result4.ok) {
    ...
    const errorWithMessage = result4.error as any;
...
Deno.test("getObjectProperty - safely extracts properties", () => {
  ...
  if (!result5.ok) {
    ...
    const errorWithMessage = result5.error as any;
    assertEquals(errorWithMessage.message?.includes("Property 'nonExistent' not found"), true);
  }
  ...
  if (!result6.ok) {
    ...
    const errorWithMessage = result6.error as any;
...
Deno.test("validateMappingResult - validates template mapping results", () => {
  ...
  if (!result4.ok) {
    ...
    const errorWithMessage = result4.error as any;
    assertEquals(errorWithMessage.message?.includes("cannot be null or undefined"), true);
  }
  ...
  if (!result5.ok) {
    ...
    const error = result5.error as any;
...
Deno.test("validateJsonParseResult - validates JSON parsing results", () => {
  ...
  if (!result3.ok) {
    assertEquals(result3.error.kind, "InvalidFormat");
    const errorWithMessage = result3.error as any;
    assertEquals(errorWithMessage.message?.includes("Expected JSON object but got primitive string"), true);
  }
...
Deno.test("hasStringKeys - checks if value has string keys", () => {
  ...
  // Edge cases
  const obj = {};
  Object.defineProperty(obj, Symbol("sym"), { value: "test", enumerable: false });
...
Deno.test("safeObjectTraversal - safely accesses nested properties", () => {
  ...
  // Valid traversals
  const result1 = safeObjectTraversal(obj, ["level1", "level2", "level3", "value"]);
  assertEquals(result1.ok, true);
  if (result1.ok) {
  ...
  if (!result5.ok) {
    assertEquals(result5.error.kind, "InvalidFormat");
    const errorWithMessage = result5.error as any;
    assertEquals(errorWithMessage.message?.includes("Property 'missing' not found"), true);
  }
  ...
  if (!result6.ok) {
    assertEquals(result6.error.kind, "InvalidFormat");
    const errorWithMessage = result6.error as any;
    assertEquals(errorWithMessage.message?.includes("Expected object at path 'top'"), true);
  }
  ...
  if (!result8.ok) {
    const errorWithMessage = result8.error as any;
  ...
  if (!result9.ok) {
    ...
    const errorWithMessage = result9.error as any;
    assertEquals(errorWithMessage.message?.includes("Expected object at path 'a.b'"), true);
  }
});
...
Deno.test("validateObjectArray - validates arrays of objects", () => {
  ...
  if (!result4.ok) {
    assertEquals(result4.error.kind, "InvalidFormat");
    const errorWithMessage = result4.error as any;
    assertEquals(errorWithMessage.message?.includes("Expected array but got object"), true);
  }
  ...
  const result6 = validateObjectArray([{ valid: true }, "invalid", { valid: true }]);
  assertEquals(result6.ok, false);
  if (!result6.ok) {
    assertEquals(result6.error.kind, "InvalidFormat");
    const errorWithMessage = result6.error as any;
  ...
  if (!result7.ok) {
    assertEquals(result7.error.kind, "InvalidFormat");
    const errorWithMessage = result7.error as any;
...
Deno.test("edge cases - empty and special values", () => {
  ...
  const deepResult = safeObjectTraversal(deepObj, ["a", "b", "c", "d", "e", "f"]);
...
Deno.test("error message formatting with context", () => {
  ...
  if (!result1.ok) {
    const errorWithMessage = result1.error as any;
  ...
  assertEquals(result2.ok, false);
  if (!result2.ok) {
    const errorWithMessage = result2.error as any;
    assertEquals(errorWithMessage.message?.includes("configuration"), true);
  }
  
  // getObjectPropertyAsObject with context
  const result3 = getObjectPropertyAsObject({ prop: "not object" }, "prop", "settings");
  assertEquals(result3.ok, false);
  if (!result3.ok) {
    const errorWithMessage = result3.error as any;
  ...
  assertEquals(result4.ok, false);
  if (!result4.ok) {
    const error = result4.error as any;
  ...
  const result6 = safeObjectTraversal({ a: "not object" }, ["a", "b"], "data.structure");
  assertEquals(result6.ok, false);
  if (!result6.ok) {
    const errorWithMessage = result6.error as any;
