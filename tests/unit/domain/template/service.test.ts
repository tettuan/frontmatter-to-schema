...
Deno.test("TemplateProcessingService - Service Layer Tests", async (t) => {
  ...
  await t.step("should process templates end-to-end", async () => {
    ...
    if (templateDef.ok) {
      const template = Template.create("e2e-test", templateDef.data, "E2E test");
      if (template.ok) {
        ...
        const result = await service.processTemplate("e2e-test", data, schema, "json");
  ...
  await t.step("should handle template not found", async () => {
    const result = await service.processTemplate("non-existent", {}, {}, "json");
  ...
  await t.step("should cache processed templates", async () => {
    ...
    if (def.ok) {
      ...
      if (template.ok) {
        ...
        // First processing
        const result1 = await service.processTemplate("cache-test", { value: "cached-value" }, {}, "json");
        assertEquals(result1.ok, true);
        
        // Second processing (should use cached template)
        const result2 = await service.processTemplate("cache-test", { value: "cached-value" }, {}, "json");
        assertEquals(result2.ok, true);
        
        if (result1.ok && result2.ok) {
          assertEquals(result1.data, result2.data, "Results should be identical");
        }
      }
    }
  });
...
Deno.test("TemplateProcessingService - Integration Tests", async (t) => {
  ...
  await t.step("should handle complex nested data structures", async () => {
    ...
    if (templateDef.ok) {
      const template = Template.create("complex-test", templateDef.data, "Complex test");
      if (template.ok) {
        await repo.save(template.data);
        ...
        const result = await service.processTemplate("complex-test", data, {}, "json");
        assertEquals(result.ok, true);
        if (result.ok) {
          const parsed = JSON.parse(result.data);
          assertEquals(parsed.project.name, "TestProject");
          assertEquals(parsed.project.dependencies.runtime, ["deno", "typescript"]);
  ...
  await t.step("should support different output formats", async () => {
    ...
    if (def.ok) {
      ...
      if (template.ok) {
        ...
        // Test YAML output
        const yamlResult = await service.processTemplate("format-test", data, {}, "yaml");
        ...
        // Test JSON output
        const jsonResult = await service.processTemplate("format-test", data, {}, "json");
  ...
  await t.step("should handle partial data gracefully", async () => {
    ...
    if (def.ok) {
      const template = Template.create("partial-test", def.data, "Partial test");
      if (template.ok) {
        await repo.save(template.data);
...
Deno.test("TemplateProcessingService - Concurrency Tests", async (t) => {
  ...
  await t.step("should handle concurrent template processing", async () => {
    ...
    if (def.ok) {
      const template = Template.create("concurrent", def.data, "Concurrent test");
      if (template.ok) {
        await repo.save(template.data);
  ...
  await t.step("should handle mixed operations concurrently", async () => {
    ...
    for (const result of results) {
      assertEquals(result.status, "fulfilled", "All operations should complete");
    }
  });
...
Deno.test("TemplateProcessingService - Error Recovery Tests", async (t) => {
  ...
  await t.step("should handle strategy errors gracefully", async () => {
    ...
    if (def.ok) {
      ...
      const template = Template.create("recovery-test", invalidDef.ok ? invalidDef.data : def.data, "Recovery test");
      if (template.ok) {
        await repo.save(template.data);
