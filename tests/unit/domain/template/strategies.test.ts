...
Deno.test("NativeTemplateStrategy - Totality Tests", async (t) => {
  ...
  await t.step("should handle JSON templates with Result type", async () => {
    ...
    if (templateDef.ok) {
      const template = Template.create("json-test", templateDef.data, "JSON test");
      if (template.ok) {
        ...
        assertEquals(result.ok, true, "Should process JSON template successfully");
        if (result.ok) {
  ...
  await t.step("should handle missing paths with Result type", async () => {
    ...
    if (templateDef.ok) {
      const template = Template.create("missing-path", templateDef.data, "Missing path test");
      if (template.ok) {
        const context: TemplateApplicationContext = {
          extractedData: {
            other: "value"
        ...
        const result = await strategy.process(template.data, context);
        assertEquals(result.ok, false, "Should fail gracefully for missing path");
        if (!result.ok) {
  ...
  await t.step("should handle YAML format conversion", async () => {
    ...
    if (templateDef.ok) {
      const template = Template.create("yaml-test", templateDef.data, "YAML test");
      if (template.ok) {
        const context: TemplateApplicationContext = {
          extractedData: {
            name: "Test",
  ...
  await t.step("should handle deeply nested templates", async () => {
    ...
    if (templateDef.ok) {
      const template = Template.create("nested-test", templateDef.data, "Nested test");
      if (template.ok) {
        const context: TemplateApplicationContext = {
          extractedData: {
            deep: {
...
Deno.test("AITemplateStrategy - Integration Tests", async (t) => {
  ...
  await t.step("should handle AI failure gracefully", async () => {
    ...
    const templateDef = TemplateDefinition.create("{{template}}", "custom");
    if (templateDef.ok) {
      const template = Template.create("ai-fail", templateDef.data, "AI fail test");
      if (template.ok) {
        const context: TemplateApplicationContext = {
          extractedData: { value: "test" },
          schema: {},
        ...
        if (!result.ok) {
          assertEquals(result.error.message.includes("AI template processing failed"), true);
        }
      }
    }
  });
  ...
  await t.step("should build proper prompts", async () => {
    ...
    const strategy = new AITemplateStrategy(mockAI);
    const templateDef = TemplateDefinition.create("Template: {{value}}", "custom");
    
    if (templateDef.ok) {
      const template = Template.create("prompt-test", templateDef.data, "Prompt test");
      if (template.ok) {
        const context: TemplateApplicationContext = {
        ...
        assertEquals(capturedPrompt.includes("テンプレートへの値の当て込み処理"), true);
...
Deno.test("CompositeTemplateStrategy - Fallback Tests", async (t) => {
  await t.step("should use primary strategy when successful", async () => {
    ...
    if (templateDef.ok) {
      const template = Template.create("composite-test", templateDef.data, "Composite test");
      if (template.ok) {
        const context: TemplateApplicationContext = {
          extractedData: { value: "test" },
          schema: {},
  ...
  await t.step("should fallback to secondary when primary fails", async () => {
    ...
    if (templateDef.ok) {
      const template = Template.create("fallback-test", templateDef.data, "Fallback test");
      if (template.ok) {
        const context: TemplateApplicationContext = {
  ...
  await t.step("should fail when both strategies fail", async () => {
    ...
    if (templateDef.ok) {
      const template = Template.create("both-fail", templateDef.data, "Both fail test");
      if (template.ok) {
        const context: TemplateApplicationContext = {
...
Deno.test("Strategy Pattern - Boundary Tests", async (t) => {
  ...
  await t.step("should handle all supported formats", async () => {
    ...
    for (const format of formats) {
      ...
      if (templateDef.ok) {
        const template = Template.create(`${format}-test`, templateDef.data, `${format} test`);
        if (template.ok) {
          assertEquals(strategy.canHandle(template.data), true, `Should handle ${format} format`);
        }
      }
    }
  });
  ...
  await t.step("should reject unsupported formats", async () => {
    ...
    if (templateDef.ok) {
      const template = Template.create("handlebars-test", templateDef.data, "Handlebars test");
      if (template.ok) {
        const context: TemplateApplicationContext = {
          extractedData: {},
          schema: {},
        ...
        if (!result.ok) {
          assertEquals(result.error.message.includes("Unsupported template format"), true);
        }
      }
    }
  });
  ...
  await t.step("should maintain immutability of input data", async () => {
    ...
    if (templateDef.ok) {
      const template = Template.create("immutable-test", templateDef.data, "Immutable test");
      if (template.ok) {
        const context: TemplateApplicationContext = {
