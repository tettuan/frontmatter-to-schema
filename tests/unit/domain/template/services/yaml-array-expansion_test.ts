...
Deno.test("VariableReplacer - YAML Array Expansion {@items} Pattern", async (t) => {
  ...
  await t.step("should expand {@items} in YAML array structure", () => {
    ...
    assert(result.ok, `Array expansion should succeed: ${result.ok ? "" : result.error.message}`);
    ...
    assert(resultData.books.length === 2, `Should have 2 items, got ${resultData.books.length}`);

    // Verify data integrity
    assert(resultData.books[0].title === "TypeScript Best Practices", "First book title should match");
    assert(resultData.books[1].title === "Domain-Driven Design", "Second book title should match");

    // Ensure no template placeholders remain
    const resultStr = JSON.stringify(resultData);
    assert(!resultStr.includes("{@items}"), "No {@items} placeholders should remain");
  });
  ...
  await t.step("should handle nested YAML array expansion", () => {
    ...
    // Assert
    assert(result.ok, `Nested expansion should succeed: ${result.ok ? "" : result.error.message}`);

    const resultData = result.data as any;
    assert(resultData.metadata.version === "1.0.0", "Metadata should be preserved");
    assert(Array.isArray(resultData.content.articles), "articles should be an array");
    assert(resultData.content.articles.length === 2, "Should have 2 articles");

    // Verify no placeholders remain
    const resultStr = JSON.stringify(resultData);
    assert(!resultStr.includes("{@items}"), "No {@items} placeholders should remain in nested structure");
  });

  await t.step("should handle multiple {@items} expansions in same template", () => {
    ...
    // Assert
    assert(result.ok, `Multiple expansion should succeed: ${result.ok ? "" : result.error.message}`);
    ...
    assert(resultData.books[0].title === "Book 1", "First book title should match");
    assert(resultData.featured[0].title === "Book 1", "First featured title should match");
  });
  ...
  await t.step("should preserve non-{@items} array elements", () => {
    ...
    // Assert
    assert(result.ok, `Mixed array expansion should succeed: ${result.ok ? "" : result.error.message}`);
    ...
    const resultStr = JSON.stringify(resultData);
    assert(!resultStr.includes("{@items}"), "No {@items} placeholders should remain");
    assert(resultStr.includes("Dynamic Book 1"), "Dynamic content should be present");
    assert(resultStr.includes("static-book"), "Static content should be preserved");
  });
  ...
  await t.step("should handle empty data array gracefully", () => {
    ...
    // Assert
    assert(result.ok, `Empty array expansion should succeed: ${result.ok ? "" : result.error.message}`);
