...
Deno.test("FileTemplateRepository - Template Loading", async (t) => {
  ...
  await t.step("loads JSON template successfully", async () => {
    ...
    assertEquals(result.data.getId(), "test-json");
    assertEquals(result.data.getDefinition().getFormat(), "json");
  ...
  await t.step("loads YAML template successfully", async () => {
    ...
    assert(result.ok);
    assertExists(result.data);
    assertEquals(result.data.getId(), "test-yaml");
    assertEquals(result.data.getDefinition().getFormat(), "yaml");
  ...
  await t.step("loads YML template successfully", async () => {
    ...
    assert(result.ok);
    assertExists(result.data);
    assertEquals(result.data.getId(), "test-yml");
    assertEquals(result.data.getDefinition().getFormat(), "yaml");
  ...
  await t.step("loads Handlebars template successfully", async () => {
    ...
    assert(result.ok);
    assertExists(result.data);
    assertEquals(result.data.getId(), "test-hbs");
    assertEquals(result.data.getDefinition().getFormat(), "handlebars");
  ...
  await t.step("loads custom template successfully", async () => {
    ...
    assert(result.ok);
    assertExists(result.data);
    assertEquals(result.data.getId(), "test-custom");
    assertEquals(result.data.getDefinition().getFormat(), "custom");
  ...
  await t.step(
    ...
    async () => {
      ...
      assert(result.ok);
      assertExists(result.data);
      assertEquals(result.data.getId(), "multi-ext");
      assertEquals(result.data.getDefinition().getFormat(), "yaml");
...
Deno.test("FileTemplateRepository - Direct Path Loading", async (t) => {
  ...
  await t.step("loadFromPath loads template successfully", async () => {
    ...
    assert(result.ok);
    assertExists(result.data);
    assertEquals(result.data.getId(), "direct-load");
...
Deno.test("FileTemplateRepository - Template Saving", async (t) => {
  ...
  await t.step("saves JSON template successfully", async () => {
    // Arrange
    await setup();
    const definition = TemplateDefinition.create(
    ...
    // Act
    const result = await repository.save(template.data);
  ...
  await t.step("saves custom template successfully", async () => {
    ...
    const definition = TemplateDefinition.create(customContent, "custom");
    assert(definition.ok);
    const template = Template.create(
      "save-custom",
      definition.data,
  ...
  await t.step("updates template cache after save", async () => {
    ...
    // Act
    const saveResult = await repository.save(template.data);
    ...
    assertEquals(loadResult.data.getId(), "cache-test");
  ...
  await t.step("handles save errors gracefully", async () => {
    ...
    // Act
    const result = await invalidRepo.save(template.data);
...
Deno.test("FileTemplateRepository - Integration Tests", async (t) => {
  ...
  await t.step("full workflow: save, load, exists, list", async () => {
    ...
    assert(loadResult.ok);
    assertEquals(loadResult.data.getId(), "workflow-test");
  ...
  await t.step("handles concurrent operations", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      assert(template.ok);
      promises.push(repository.save(template.data));
    }
  ...
  await t.step(
    ...
    async () => {
      ...
      const specialContent = JSON.stringify({
        title: "Template with 特殊文字",
        description: "Content with emojis 🚀 and\nnewlines\tand tabs",
        unicode: "🌟 ⭐ ✨",
      });
      const definition = TemplateDefinition.create(specialContent, "json");
      assert(definition.ok);
      const template = Template.create(
        "special-chars",
        definition.data,
      ...
      // Assert
      assert(loadResult.ok);
      assertEquals(loadResult.data.getId(), "special-chars");
      assertStringIncludes(
        loadResult.data.getDefinition().getDefinition(),
        "特殊文字",
      );
      assertStringIncludes(
        loadResult.data.getDefinition().getDefinition(),
  ...
  await t.step("handles large template content", async () => {
    ...
    // Assert
    assert(loadResult.ok);
    assertEquals(loadResult.data.getId(), "large-template");
    assertStringIncludes(
      loadResult.data.getDefinition().getDefinition(),
      "prop999",
    );
