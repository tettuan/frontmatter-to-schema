...
ドメイン情報は、 `docs/domain/design_domain_boundary-20250802.md` および
`docs/domain/architecture/subdomain_01_stock/**/*.md` を必ず読むこと。
プロジェクト構造は、 `docs/architecture/domain-driven-implementation.md`
に記載がある。

AI実装複雑化防止フレームワーク(`docs/development/ai-complexity-control_compact.ja.md`)に則り、エントロピー増大を抑制すること。

テスト方針: テストに関しては `docs/tests/README.md`,
`docs/tests/testing_guidelines.md` を読むこと。
...
初期タスク（仕様理解、実装調査）を早々に実施します。
続いて、仕様の理解を進め、実装方針に基づいて進めてください。

今なにをすべきか（タスク分割や、状況整理、要件定義、実装、テスト）について、把握していることが重要です。今やる詳細タスクへ分割し、タスク化したうえで進めてください。
