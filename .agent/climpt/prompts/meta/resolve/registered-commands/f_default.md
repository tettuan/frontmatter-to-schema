...
内容を読み、「使いたい内容」に相応しい用途のものを探し出すこと。
相応しい内容がない場合は、「」
...
1. 「使いたい内容」からメインテーマを決め、climpt-* を選ぶ
2. 「使いたい内容」を input_text, input_text_file のいずれで指定するか決める。
3. -iや-aでプロンプトを切り替えるか判断する。
4. uv-* オプションを用いるか決める。
...
なお、オプションの値は "=" 結合である。(not "-o tmp/outfile", but "-o=tmp/outfile")
...
「clear 判断」したあと、推奨する実行コマンドと実行指示のみを出力する。


## clear 判断

全てのタスクが完了していた場合: チャット履歴クリアする。その後「出力形式」で出力する
全てのタスクが完了していない場合: クリアせず「出力形式」で出力する
