    ...
    if (
      ...
      return {
        ...
        error: {
          ...
          message: "Schema must be JSON or YAML",
        },
      };
    }
...
export class SchemaDefinition {
  ...
  static create(
    ...
    if (!definition || typeof definition !== "object") {
      return {
        ok: false,
        error: { kind: "InvalidDefinition", message: "Schema must be object" },
      };
    }
    ...
    if (!isValidJsonSchema(definition)) {
      return {
        ok: false,
        error: { kind: "InvalidSchema", message: "Invalid JSON Schema" },
      };
    }

    return {
      ok: true,
      data: new SchemaDefinition(definition as object, metadata),
    };
  }

  getDefinition(): Readonly<object> {
...
export class SchemaVersion {
  ...
  static create(version: string): Result<SchemaVersion, VersionError> {
    ...
    if (!match) {
      return {
        ok: false,
        error: { kind: "InvalidFormat", message: "Version must be X.Y.Z" },
      };
    }
...
export class SchemaRepository {
  ...
  async load(path: SchemaPath): Promise<Result<Schema, RepositoryError>> {
    ...
    if (!contentResult.ok) {
      return {
        ok: false,
        error: { kind: "LoadError", message: contentResult.error.message },
      };
    }
    ...
    try {
      ...
      return {
        ok: false,
        error: { kind: "ParseError", message: `Failed to parse: ${e}` },
      };
    }
  ...
  async save(
    ...
    const content = JSON.stringify(
      schema.getDefinition().getDefinition(),
      null,
      2,
    );
...
export class TemplateFormat {
  ...
  static create(format: string): Result<TemplateFormat, FormatError> {
    ...
    const formatType = normalized === "yml"
      ? "yaml"
      : normalized === "hbs"
      ? "handlebars"
      : normalized as any;
...
export class TemplateContent {
  ...
  static create(content: string): Result<TemplateContent, ContentError> {
    if (!content) {
      return {
        ok: false,
        error: { kind: "EmptyContent", message: "Template cannot be empty" },
      };
    }
...
export class TemplateRepository {
  ...
  async load(
    path: TemplatePath,
  ): Promise<Result<AnalysisTemplate, RepositoryError>> {
    // ファイル読み込み
    const contentResult = await this.fileSystem.readFile(path.getValue());
    if (!contentResult.ok) {
      return {
    ...
    if (!formatResult.ok) {
      return {
        ok: false,
        error: { kind: "InvalidFormat", message: formatResult.error.message },
      };
    }
    ...
    if (!templateContentResult.ok) {
      return {
        ...
        error: {
          kind: "InvalidContent",
          message: templateContentResult.error.message,
        },
      };
    }
    ...
    return AnalysisTemplate.create(
      name,
      formatResult.data,
...
export class OutputPath {
  ...
  static create(path: string): Result<OutputPath, PathError> {
    if (!path) {
      return {
        ok: false,
        error: { kind: "EmptyPath", message: "Output path cannot be empty" },
      };
    }
...
export class FinalResult {
  ...
  getResults(): ReadonlyMap<DocumentId, StructuredData> {
    return this.results;
  }
    ...
    try {
      switch (this.format) {
        ...
        default:
          return {
            ok: false,
            error: { kind: "UnsupportedFormat", format: this.format },
          };
      }
    } catch (e) {
      return {
        ok: false,
        error: { kind: "SerializationFailed", message: `${e}` },
      };
    }
  }
}
...
export class ResultAggregator {
  ...
  integrate(
    ...
    data: StructuredData,
  ): Result<FinalResult, IntegrationError> {
  ...
  async save(
    ...
    if (!this.currentResult) {
      return {
        ok: false,
        error: { kind: "NotInitialized", message: "No result to save" },
      };
    }
