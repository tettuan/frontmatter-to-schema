...
##### P1.3: AI情報抽出サブプロセス（1回目のclaude -p）
- **誰が**: Claude API（`claude -p`コマンド - 1回目）
- **何を**: 抽出されたフロントマターデータ（成果B）
- **どのように**: プロンプトAを使用して、フロントマターと解析結果のSchemaから情報を抽出
- **入力**: 成果B（フロントマター） + 解析結果のSchema + プロンプトA
- **出力**: 成果C（抽出された情報）
- **MoSCoW**: Must Have（必須）
- **理由**: 柔軟な解析の第1段階

##### P1.4: テンプレート当て込みサブプロセス（2回目のclaude -p）
- **誰が**: Claude API（`claude -p`コマンド - 2回目）
- **何を**: AI抽出結果（成果C）
- **どのように**: プロンプトBを使用して、抽出情報を解析テンプレートへ当て込む
- **入力**: 成果C（抽出された情報） + 解析結果のSchema + プロンプトB
- **出力**: 成果D（構造化されたデータ）
- **MoSCoW**: Must Have（必須）
