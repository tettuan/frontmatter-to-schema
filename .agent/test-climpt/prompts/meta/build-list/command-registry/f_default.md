...
1. `.agent/climpt/registered-commands.md` を読む
2. 「実行可能なコマンド」を `.deno/bin/climpt-*`
   から取得する(「段落リスト」とする)
3. 段落リストをもとに、`.agent/climpt/registered-commands.md` の段落に記載する
   3-1. すでにあれば記載不要、なければ段落を新説 3-2. 重複があれば統合

### 実行コマンド一覧作成

4. 「プロンプト」からファイル一覧を取得する（「実行コマンド一覧」とする）

### 実行コマンドの説明記述

実行コマンド一覧を1つずつ step by step 処理する

5. 「プロンプト配置ルール」に従い、段落へコマンドを記載する 5-1.
   プロンプト単位で1行とする
6. プロンプトのテンプレート内部を読み、オプションを特定する 6-1.
   テンプレート内部の {variable} パターンを取得する 6-2.
   「オプション一覧」をもとに、利用可能な変数からオプションを判断する 6-3.
   「Notice:
   この指示書をAgentは選択はしない。」と記載ある場合は、記載せず削除する。
7. コマンドの列に使用可を記載する

#### 詳細説明の作成

8. フロントマターがあれば、title, description, usage, variables を取得する
9. 一覧表の下に、取得したフロントマターを記載する。
...
`.agent/climpt/registered-commands.md`
